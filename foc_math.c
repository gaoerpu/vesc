...
#include <math.h>
//该文件实现FOC控制算法中的数学计算函数，包括坐标变换、svpwm、滤波器和观测器等
...
void foc_observer_update(float v_alpha, float v_beta, float i_alpha, float i_beta,
	...
	// Saturation compensation   3种饱和补偿方式用于补偿电感

	/*SAT_COMP_LAMBDA：基于磁链变化来调整电感，理论上更精确，但假设不一定成立。

	SAT_COMP_FACTOR：基于电流比例的补偿，简单易实现，但可能不够精确。

	SAT_COMP_LAMBDA_AND_FACTOR：结合了磁链和电流补偿，精度较高，但计算较为复杂。
	在选择补偿方式时，需要根据实际应用的负载特性来决定。
	如果磁链变化较为明显且能精确估算，第一种方法可能更合适；
	如果电流变化显著对电感有影响，第二种方法或第三种方法可能更有效*/

	switch(conf_now->foc_sat_comp_mode) {
	case SAT_COMP_LAMBDA:        //用LAMBDA的方式补偿电感，就是估计的LAMBDA的变化来补偿电感
	...
	case SAT_COMP_LAMBDA_AND_FACTOR: {  //融合上面2个方法去补偿
		if (conf_now->foc_observer_type >= FOC_OBSERVER_ORTEGA_LAMBDA_COMP ||
				conf_now->foc_observer_type >= FOC_OBSERVER_MXLEMMING_LAMBDA_COMP ||
				conf_now->foc_observer_type >= FOC_OBSERVER_MXV_LAMBDA_COMP ||
				conf_now->foc_observer_type >= FOC_OBSERVER_MXV_LAMBDA_COMP_LIN) {
	...
	//上面都是准备工作，下面开始观测器的主体计算

	float L_ia = L * i_alpha;  //电流产生的磁链
	float L_ib = L * i_beta;
	const float R_ia = R * i_alpha;  //电阻产生的电压
	const float R_ib = R * i_beta;
	const float gamma_half = motor->m_gamma_now * 0.5; //观测器增益

	switch (conf_now->foc_observer_type) {
	case FOC_OBSERVER_ORTEGA_ORIGINAL: {   //默认算法
	...
	if (mag < (lambda * 0.5)) {
		state->x1 *= 1.1;    //如果估计的磁链小于磁链的一半，就乘1.1。原因：防止幅度过低，因为这会使角度非常不稳定。
		state->x2 *= 1.1;
	}

	if (phase) {
		*phase = utils_fast_atan2(state->x2 - L_ib, state->x1 - L_ia);  //永磁体产生磁链的分量得到相位，后面在经过锁相环
	}            //反正切？？？？？？
